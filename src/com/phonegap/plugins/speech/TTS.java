	...
	public PluginResult execute(String action, JSONArray args, String callbackId) {
		...
		try {
			if (action.equals("speak")) {
				...
				if (isReady()) {
					mTts.playSilence(args.getLong(0), TextToSpeech.QUEUE_ADD,
							null);
					return new PluginResult(status, result);
				} else {
					JSONObject error = new JSONObject();
				...
				if (isReady()) {
					float speed = (float) (args.optLong(0, 100))
							/ (float) 100.0;
				...
				if (isReady()) {
					float pitch = (float) (args.optLong(0, 100))
							/ (float) 100.0;
					mTts.setPitch(pitch);
					return new PluginResult(status, result);
				} else {
				...
				if (mTts == null) {
					...
					mTts = new TextToSpeech(cordova.getActivity()
		...
		if (status == TextToSpeech.SUCCESS) {
			...
			try {
				...
				PluginResult error = new PluginResult(
						PluginResult.Status.ERROR, TTS.STOPPED);
				error.setKeepCallback(false);
				this.error(error, this.startupCallbackId);
			}
